/**
 * Summary: This is a UI-card that contains the table for counties from grafana.
 * 
 * Description: The table for counties displays the total number of infections, recovered cases and
 * of deaths for each county since the start of data collection. The start of data collection can
 * be retrieved from the data base or directly from the data source. 
 * 
...
const URL_MID_TABLE = "/d-solo/aim5ADoGz/total-germany-counties-states?orgId=1&panelId=32"
...
interface TablecardCtyProps {
  panelTheme?: string | ""
  panelHeight?: string
}

export const TablecardCty = (props: TablecardCtyProps) => {
  const { panelTheme, panelHeight } = props
  const title = `tableCty`
    ...
    <div className={classes.root}>
      ...
      <TabPanel value={value} index={1}>
        <Typography color="textSecondary" component="p" variant="h5">
          - Infektionen: Gesamtzahl der Infektionen in einem Landkreis <br />
          - Genesene: Gesamtzahl der Genesenen in einem Landkreis<br />
          - Todesfälle: Gesamtzahl der Todesfälle in einem Landkreis<br />
          ...
          Durch Klicken des Filtersymbols lassen sich Einträge ein- und ausblenden.
        </Typography>
      </TabPanel>
    </div>
  );
